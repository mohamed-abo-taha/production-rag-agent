...
, the maximum likelihood estimate (MLE) of the unknown parameters is determined by maximizing the marginal likelihood of the observed data
    ...
    {\displaystyle {\begin{aligned}L({\boldsymbol {\theta }};\mathbf {X} )=p(\mathbf {X} \mid {\boldsymbol {\theta }})&=\int p(\mathbf {X} ,\mathbf {Z} \mid {\boldsymbol {\theta }})\,d\mathbf {Z} \\&=\int p(\mathbf {X} \mid \mathbf {Z} ,{\boldsymbol {\theta }})p(\mathbf {Z} \mid {\boldsymbol {\theta }})\,d\mathbf {Z} \end{aligned}}}
...
More succinctly, we can write it as one equation:
    ...
    {\displaystyle {\boldsymbol {\theta }}^{(t+1)}=\mathop {\arg \max } _{\boldsymbol {\theta }}\operatorname {E} _{\mathbf {Z} \sim p(\cdot |\mathbf {X} ,{\boldsymbol {\theta }}^{(t)})}\left[\log p(\mathbf {X} ,\mathbf {Z} |{\boldsymbol {\theta }})\right]\,}
...
. The left-hand side is the expectation of a constant, so we get:
    ...
    {\displaystyle {\begin{aligned}\log p(\mathbf {X} \mid {\boldsymbol {\theta }})&=\sum _{\mathbf {Z} }p{\left(\mathbf {Z} \mid \mathbf {X} ,{\boldsymbol {\theta }}^{(t)}\right)}\log p(\mathbf {X} ,\mathbf {Z} \mid {\boldsymbol {\theta }})-\sum _{\mathbf {Z} }p{\left(\mathbf {Z} \mid \mathbf {X} ,{\boldsymbol {\theta }}^{(t)}\right)}\log p(\mathbf {Z} \mid \mathbf {X} ,{\boldsymbol {\theta }})\\&=Q({\boldsymbol {\theta }}\mid {\boldsymbol {\theta }}^{(t)})+H({\boldsymbol {\theta }}\mid {\boldsymbol {\theta }}^{(t)}),\end{aligned}}}
...
⁠: 
    ...
    {\displaystyle q^{(t)}=\mathop {\arg \max } _{q}F{\left(q,\theta ^{(t)}\right)}}
...
⁠: 
    ...
    {\displaystyle \theta ^{(t+1)}=\mathop {\arg \max } _{\theta }F{\left(q^{(t)},\theta \right)}}
...
where 
 ...
 are scalar state estimates calculated by a filter or a smoother. The updated model coefficient estimate is obtained via
    ...
    {\displaystyle {\widehat {F}}={\frac {\sum \limits _{k=1}^{N}{\left({\widehat {x}}_{k+1}-{\widehat {F}}{\widehat {x}}_{k}\right)}^{2}}{\sum \limits _{k=1}^{N}{\widehat {x}}_{k}^{2}}}.}
...
, and let 
 ...
 be the latent variables that determine the component from which the observation originates.
    ...
    {\displaystyle {\begin{aligned}X_{i}\mid (Z_{i}=1)&\sim {\mathcal {N}}_{d}({\boldsymbol {\mu }}_{1},\Sigma _{1}),\\X_{i}\mid (Z_{i}=2)&\sim {\mathcal {N}}_{d}({\boldsymbol {\mu }}_{2},\Sigma _{2}),\end{aligned}}}
...
where
    ...
    {\displaystyle \operatorname {P} (Z_{i}=1)=\tau _{1}\,\quad {\text{and}}\quad \operatorname {P} (Z_{i}=2)=\tau _{2}=1-\tau _{1}.}
...
and the complete-data likelihood function is
    ...
    {\displaystyle {\begin{aligned}L(\theta ;\mathbf {x} ,\mathbf {z} )&=p(\mathbf {x} ,\mathbf {z} \mid \theta )\\&=\prod _{i=1}^{n}\prod _{j=1}^{2}\left[f(\mathbf {x} _{i};{\boldsymbol {\mu }}_{j},\Sigma _{j})\tau _{j}\right]^{\mathbb {I} (z_{i}=j)},\end{aligned}}}
...
or
    ...
    {\displaystyle \log L(\theta ;\mathbf {x} ,\mathbf {z} )=\sum _{i=1}^{n}\sum _{j=1}^{2}\mathbb {I} (z_{i}=j)\left[\log \tau _{j}-{\tfrac {1}{2}}\log |\Sigma _{j}|-{\tfrac {1}{2}}(\mathbf {x} _{i}-{\boldsymbol {\mu }}_{j})^{\top }\Sigma _{j}^{-1}(\mathbf {x} _{i}-{\boldsymbol {\mu }}_{j})-{\tfrac {d}{2}}\log(2\pi )\right],}
...
Given our current estimate of the parameters θ(t), the conditional distribution of the Zi is determined by Bayes' theorem to be the proportional height of the normal density weighted by τ:
    ...
    {\displaystyle {\begin{aligned}T_{j,i}^{(t)}:={}&\operatorname {P} (Z_{i}=j\mid X_{i}=\mathbf {x} _{i};\theta ^{(t)})\\={}&{\frac {\tau _{j}^{(t)}\,f{\left(\mathbf {x} _{i};{\boldsymbol {\mu }}_{j}^{(t)},\Sigma _{j}^{(t)}\right)}}{\tau _{1}^{(t)}\,f{\left(\mathbf {x} _{i};{\boldsymbol {\mu }}_{1}^{(t)},\Sigma _{1}^{(t)}\right)}+\tau _{2}^{(t)}\,f{\left(\mathbf {x} _{i};{\boldsymbol {\mu }}_{2}^{(t)},\Sigma _{2}^{(t)}\right)}}}.\end{aligned}}}
...
:
    ...
    {\displaystyle {\begin{aligned}{\boldsymbol {\tau }}^{(t+1)}&=\mathop {\arg \max } _{\boldsymbol {\tau }}Q{\left(\theta \mid \theta ^{(t)}\right)}\\&=\mathop {\arg \max } _{\boldsymbol {\tau }}\left\{\left[\sum _{i=1}^{n}T_{1,i}^{(t)}\right]\log \tau _{1}+\left[\sum _{i=1}^{n}T_{2,i}^{(t)}\right]\log \tau _{2}\right\}.\end{aligned}}}
...
This has the same form as the maximum likelihood estimate for the binomial distribution, so
    ...
    {\displaystyle \tau _{j}^{(t+1)}={\frac {\sum \limits _{i=1}^{n}T_{j,i}^{(t)}}{\sum \limits _{i=1}^{n}\left(T_{1,i}^{(t)}+T_{2,i}^{(t)}\right)}}={\frac {1}{n}}\sum _{i=1}^{n}T_{j,i}^{(t)}.}
...
:
    ...
    {\displaystyle {\begin{aligned}\left({\boldsymbol {\mu }}_{1}^{(t+1)},\Sigma _{1}^{(t+1)}\right)&=\mathop {\arg \max } _{{\boldsymbol {\mu }}_{1},\Sigma _{1}}Q{\left(\theta \mid \theta ^{(t)}\right)}\\&=\mathop {\arg \max } _{{\boldsymbol {\mu }}_{1},\Sigma _{1}}\sum _{i=1}^{n}T_{1,i}^{(t)}\left\{-{\tfrac {1}{2}}\log \left|\Sigma _{1}\right|-{\tfrac {1}{2}}\left(\mathbf {x} _{i}-{\boldsymbol {\mu }}_{1}\right)^{\top }\Sigma _{1}^{-1}\left(\mathbf {x} _{i}-{\boldsymbol {\mu }}_{1}\right)\right\}.\end{aligned}}}
...
This has the same form as a weighted maximum likelihood estimate for a normal distribution, so
    ...
    {\displaystyle {\begin{aligned}{\boldsymbol {\mu }}_{1}^{(t+1)}&={\frac {\sum \limits _{i=1}^{n}T_{1,i}^{(t)}\mathbf {x} _{i}}{\sum \limits _{i=1}^{n}T_{1,i}^{(t)}}},\\[1ex]\Sigma _{1}^{(t+1)}&={\frac {\sum \limits _{i=1}^{n}T_{1,i}^{(t)}\left(\mathbf {x} _{i}-{\boldsymbol {\mu }}_{1}^{(t+1)}\right)\left(\mathbf {x} _{i}-{\boldsymbol {\mu }}_{1}^{(t+1)}\right)^{\top }}{\sum \limits _{i=1}^{n}T_{1,i}^{(t)}}}\end{aligned}}}
...
and, by symmetry,
    ...
    {\displaystyle {\begin{aligned}{\boldsymbol {\mu }}_{2}^{(t+1)}&={\frac {\sum \limits _{i=1}^{n}T_{2,i}^{(t)}\mathbf {x} _{i}}{\sum \limits _{i=1}^{n}T_{2,i}^{(t)}}},\\[1ex]\Sigma _{2}^{(t+1)}&={\frac {\sum \limits _{i=1}^{n}T_{2,i}^{(t)}\left(\mathbf {x} _{i}-{\boldsymbol {\mu }}_{2}^{(t+1)}\right)\left(\mathbf {x} _{i}-{\boldsymbol {\mu }}_{2}^{(t+1)}\right)^{\top }}{\sum \limits _{i=1}^{n}T_{2,i}^{(t)}}}.\end{aligned}}}
  


==== Termination ====
Conclude the iterative process if the expectation update is sufficiently small. That is, if 
    ...
    {\displaystyle \left|\operatorname {E} _{Z\mid \theta ^{(t)},\mathbf {x} }[\log L(\theta ^{(t)};\mathbf {x} ,\mathbf {Z} )]-\operatorname {E} _{Z\mid \theta ^{(t-1)},\mathbf {x} }[\log L(\theta ^{(t-1)};\mathbf {x} ,\mathbf {Z} )]\right|\leq \varepsilon }
