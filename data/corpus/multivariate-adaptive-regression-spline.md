...
Source: Wikipedia (https://en.wikipedia.org/wiki/Multivariate_adaptive_regression_spline)
...
A linear model for the above data is
    ...
    {\displaystyle {\widehat {y}}=-37+5.1x}
...
The hat on the 
 ...
 is estimated from the data.  The figure on the right shows a plot of this function: 
a line giving the predicted 
...
For example, each line in the formula for ozone above is one basis function multiplied by its coefficient.
Each basis function 
...
a constant 1. There is just one such term, the intercept. In the ozone formula above, the intercept term is 5.2.
a hinge function. A hinge function has the form  
...
a product of two or more hinge functions. These basis functions can model interaction between two or more variables.
