...
Source: Wikipedia (https://en.wikipedia.org/wiki/Paraphrasing_%28computational_linguistics%29)
...
, which is equivalent to 
    ...
    {\displaystyle \Pr(e_{2}|f)\Pr(f|e_{1})}
...
-dimensional word embeddings as input and produce an 
...
-dimensional vector as output. The same autoencoder is applied to every pair of words in 
 ...
 to produce 
    ...
    {\displaystyle \lfloor m/2\rfloor }
...
Given two sentences 
 ...
 of length 4 and 3 respectively, the autoencoders would produce 7 and 5 vector representations including the initial word embeddings. The euclidean distance is then taken between every combination of vectors in 
 ...
 to produce a similarity matrix 
    ...
    {\displaystyle S\in \mathbb {R} ^{7\times 5}}
...
. 
 ...
 is then subject to a dynamic min-pooling layer to produce a fixed size 
 ...
 roughly even sections. The output is then normalized to have mean 0 and standard deviation 1 and is fed into a fully connected layer with a softmax output. The dynamic pooling to softmax model is trained using pairs of known paraphrases.
