...
Source: Wikipedia (https://en.wikipedia.org/wiki/Hierarchical_Risk_Parity)
...
Next, a secondary distance matrix 
 ...
 is computed, where each entry measures the Euclidean distance between the distance profiles of two assets:
    ...
    {\displaystyle {\tilde {d}}_{i,j}={\sqrt {\sum _{n=1}^{N}(d_{n,i}-d_{n,j})^{2}}}}
...
. The distance between this new cluster and all remaining items 
 ...
 is updated using a linkage criterion. One example is the single linkage (nearest neighbor) method:
    ...
    {\displaystyle {\dot {d}}_{k,u[1]}=\min {{\tilde {d}}_{k,l}\;l\in u[1]}}
