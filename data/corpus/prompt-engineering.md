...
Source: Wikipedia (https://en.wikipedia.org/wiki/Prompt_engineering)
...
=== Multi-shot ===
A prompt may include a few examples for a model to learn from in-context, an implementation of few-shot learning for LLMs. For example, the prompt may ask the model to complete "maison → house, chat → cat, chien →", with the expected response being dog.
